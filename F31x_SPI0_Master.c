...
#define SPI_CLOCK          5000000      // Maximum SPI clock
                                       // The SPI clock is a maximum of 5MHz
...
#define  DATA_SAMPLE    		20     //Number of values used to evaluate rotation speed
...
void main (void)
{
  ...
  //The Z value is stored in values[4] and values[5].
	 ...
	 if(iter==DATA_SAMPLE)
	 {
		...
		{
			moyenne+=(long)speed[count];
		}
		moyenne/=DATA_SAMPLE;
		moyenne*=9.81;
		moyenne/=0.046;
		moyenne/=1000;
		moyenne=sqrt(moyenne);
		iter=0;

...
#define  DATA_SAMPLE    		500     //Number of values sampled to evaluate rotation speed
...
void PORT_Init (void)
{
   ...
   // Make color select push-pull and LED select open drain for LED control System              
   P2MDOUT = 0xF0; 
...
void sampleAcceleration (void)
{
   ...
   long accel[2]={0,0};
   unsigned long norme;
   unsigned long numerateur, denumerateur;
   unsigned int count;
	                            // spin forever
  ...
  //The Z value is stored in values[4] and values[5].
	...
	//Convert those values in mg (mg=10^-3*gravity)
	 ...
	 {
		moyenne/=DATA_SAMPLE;
		numerateur=SYSCLK/12;
		denumerateur=(sqrt(((moyenne*GRAVITY)/RADIUS)/1000)*WHEELSECTORS)/(2*PI);
		count=(unsigned int)(numerateur/denumerateur);
